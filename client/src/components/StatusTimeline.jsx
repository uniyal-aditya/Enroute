...
export default function StatusTimeline({ status }) {
  ...
  if (isRejected) {
    return (
      <div className="rounded-xl border border-red-500/30 bg-red-500/10 p-3 flex items-center gap-2.5 text-xs text-red-300">
        <AlertCircle className="h-4 w-4 text-red-400 shrink-0" />
        <div>
          <span className="font-bold text-white">Booking Request Declined:</span> The driver is unable to take this shipment. You may request space on another route.
        </div>
      </div>
    )
  }
  ...
  if (isCancelled) {
    return (
      <div className="rounded-xl border border-slate-700 bg-slate-800/80 p-3 flex items-center gap-2.5 text-xs text-slate-400">
        <AlertCircle className="h-4 w-4 text-slate-500 shrink-0" />
        <div>
          <span className="font-bold text-slate-300">Route Cancelled:</span> This trip was cancelled by the driver.
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="py-2">
      <div className="flex items-center justify-between relative">
        ...
        <div className="absolute left-4 right-4 top-3.5 -translate-y-1/2 h-0.5 bg-slate-800 -z-0">
          <div
              ...
              <div
                className={`flex h-7 w-7 items-center justify-center rounded-full text-xs font-bold transition-all duration-300 ${
                  isDone
                    ...
                    : isCurrent
                    ? 'bg-blue-600 text-white shadow-md shadow-blue-500/30 ring-4 ring-blue-500/20 animate-pulse'
